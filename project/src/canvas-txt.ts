...
const canvasTxt = {
  ...
  drawText: function (ctx: CanvasRenderingContext2D, mytext: string, x: number, y: number, width: number, height: number) {
    // Parse all to integers
    ;[x, y, width, height] = [x, y, width, height].map((el) => parseInt(el.toString()))
      ...
      if (textwidth <= width) {
        ...
        while (textwidth > linelen) {
          ...
          if (split(temptext).slice(textlen,textlen+1).join("") !== ' ') {
            while (split(temptext).slice(textlen,textlen+1).join("") !== ' ' && textlen !== 0) {
              textlen--
            }
            if (textlen === 0) {
              textlen = backup
   ...
   */
  justifyLine: function (ctx: CanvasRenderingContext2D, line: string, spaceWidth: number, spaceChar: string, width: number) {
